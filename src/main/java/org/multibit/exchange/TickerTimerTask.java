...
public class TickerTimerTask extends TimerTask {
     ...
     */
    public TickerTimerTask(ExchangeController exchangeController, FastcoinWalletFrame mainFrame, boolean isFirstExchange) {
    ...
    public void run() {
        ...
        try {
            ...
            if (marketDataService != null) {
                    ...
                    if (!Boolean.FALSE.toString().equals(controller.getModel().getUserPreference(ExchangeModel.TICKER_SHOW))
                        ...
                        for (CurrencyPair loopSymbolPair : exchangeSymbols) {
                            if (ExchangeData.OPEN_EXCHANGE_RATES_EXCHANGE_NAME.equals(shortExchangeName)) {
                                ...
                                if ("FST".equals(loopSymbolPair.baseCurrency) && loopSymbolPair.counterCurrency.equals(currency)) {
                                    getItFromTheServer = true;
                                    currencyPairToUse = loopSymbolPair;
                                    break;
                                }
                                if ("FST".equals(loopSymbolPair.counterCurrency) && loopSymbolPair.baseCurrency.equals(currency)) {
                                    getItFromTheServer = true;
                                    invertedRates = true;
                                    currencyPairToUse = loopSymbolPair;
                                    break;
                        ...
                        if (getItFromTheServer) {
                            ...
                            if (ExchangeData.OPEN_EXCHANGE_RATES_EXCHANGE_NAME.equals(shortExchangeName)) {
                                ...
                                Ticker fstUsdTicker = null;
                                log.debug("Getting fstUsdTicker for FST/USD");
                                fstUsdTicker = marketDataService.getTicker(CurrenciesLocal.FST, Currencies.USD);
                                System.out.println("fstUsdTicker = " + fstUsdTicker);

                                BigMoney usdFstRateMoney = fstUsdTicker.getLast();
                                BigDecimal usdFstRate = null;
                                if (usdFstRateMoney != null) {
                                    usdFstRate = usdFstRateMoney.getAmount();
                                    if (loopTicker.getLast() != null) {
                                        last = loopTicker.getLast().dividedBy(usdFstRate, RoundingMode.HALF_EVEN);
                                    }
                                    if (loopTicker.getBid() != null) {
                                        bid = loopTicker.getBid().dividedBy(usdFstRate, RoundingMode.HALF_EVEN);
                                    }
                                    if (loopTicker.getAsk() != null) {
                                        ask = loopTicker.getAsk().dividedBy(usdFstRate, RoundingMode.HALF_EVEN);
                                    }
                                }
                            } else {
                                ...
                                if (invertedRates) {
                                    if (reverseRates) {
                                        // USD/ FST, reciprocal rate
                                        currency = currencyPairToUse.baseCurrency;
                                    } else {
                                        // FST/ USD, reciprocal rate
                            ...
                            if (isFirstExchange) {
                                ...
                                if (ExchangeData.FASTCOIN_CHARTS_EXCHANGE_NAME.equals(shortExchangeName)) {
                ...
                // Fire exchange rate data changed - used by rest of MultiBitWallet.
            ...
            if (exchangeSymbols != null) {
                ...
                for (int i = 0; i < exchangeSymbols.size(); i++) {
                    ...
                    if (ExchangeData.OPEN_EXCHANGE_RATES_EXCHANGE_NAME.equals(newExchangeName)) {
                        if ("USD".equalsIgnoreCase(baseCurrency) && !"FST".equalsIgnoreCase(counterCurrency)) {
                        ...
                        if ("USD".equalsIgnoreCase(counterCurrency) && !"FST".equalsIgnoreCase(baseCurrency)) {
                        ...
                        if ("FST".equalsIgnoreCase(baseCurrency)) {
                            availableCurrencies.add(counterCurrency);
                        }
                        if ("FST".equalsIgnoreCase(counterCurrency)) {
                            availableCurrencies.add(baseCurrency);
                        }
                    }
                }
     ...
     * FastcoinChartsExchange.class.getName();
     * 
    ...
    private Exchange createExchange(String exchangeShortname) {
        ...
        try {
            ...
            // but error should not spread out from here to rest of MultiBitWallet.
            log.error(e.getClass().getName() + " " + e.getMessage());
