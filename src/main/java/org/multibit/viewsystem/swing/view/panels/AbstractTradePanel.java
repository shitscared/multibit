...
import org.multibit.viewsystem.dataproviders.FastcoinFormDataProvider;
...
/**
 * Abstract parent class for SendFastcoinPanel and ReceiveFastcoinPanel
 * 
 * @author jim
 * 
 */
public abstract class AbstractTradePanel extends JPanel implements Viewable, CopyQRCodeImageDataProvider, FastcoinFormDataProvider, CurrencyConverterListener {
    ...
    protected FastcoinWalletFrame mainFrame;

    protected final Controller controller;
    protected final FastcoinController fastcoinController;

    protected FastcoinWalletTextArea labelTextArea;

    protected FastcoinWalletTextField amountFSTTextField;
    protected FastcoinWalletTextField amountFiatTextField;
    protected FastcoinWalletLabel amountEqualsLabel;
    protected FastcoinWalletLabel amountUnitFiatLabel;
    
    protected Money parsedAmountFST = null;
    ...
    protected FastcoinWalletTextField addressTextField;

    protected int selectedAddressRowModel;

    protected SelectionListener addressesListener;

    protected FastcoinWalletButton createNewButton;
    protected FastcoinWalletButton deleteButton;
    ...
    protected FastcoinWalletLabel qrCodeLabel;
    ...
    protected FastcoinWalletLabel displayUsingLabel;
    protected FastcoinWalletButton copyQRCodeImageButton;
    protected FastcoinWalletButton pasteSwatchButton;
    protected FastcoinWalletButton zoomButton;
    ...
    protected FastcoinWalletButton sidePanelButton;
    ...
    public AbstractTradePanel(FastcoinWalletFrame mainFrame, FastcoinController fastcoinController) {
        ...
        try {
            setFont(FontSizer.INSTANCE.getAdjustedDefaultFontWithDelta(2 * ColorAndFontConstants.FASTCOIN_WALLET_LARGE_FONT_INCREASE));
    ...
    /**
     * is it the receive fastcoin panel (return true) or the send fastcoin panel
     * (return false)
     */
    protected abstract boolean isReceiveFastcoin();
    ...
    protected int calculateStentWidth() {
        String[] keys = new String[] { "sendFastcoinPanel.addressLabel", "sendFastcoinPanel.labelLabel",
                "sendFastcoinPanel.amountLabel", "receiveFastcoinPanel.addressLabel", "receiveFastcoinPanel.labelLabel",
                "receiveFastcoinPanel.amountLabel" };

        return FastcoinWalletTitledPanel.calculateStentWidthForKeys(controller.getLocaliser(), keys, this) + STENT_DELTA;
    }
    ...
    protected void initUI() {
        ...
        String showSidePanelText = controller.getModel().getUserPreference(FastcoinModel.SHOW_SIDE_PANEL);
    ...
    protected void createFormPanelStentsAndForcers(JPanel panel, GridBagConstraints constraints) {
        ...
        panel.add(FastcoinWalletTitledPanel.createStent(calculateStentWidth(), separatorSize), constraints);
        ...
        panel.add(FastcoinWalletTitledPanel.createStent(FastcoinWalletTitledPanel.SEPARATION_BETWEEN_NAME_VALUE_PAIRS, separatorSize),
                constraints);
        ...
        constraints.anchor = GridBagConstraints.CENTER;
        panel.add(
                FastcoinWalletTitledPanel.createStent(FastcoinWalletTitledPanel.SEPARATION_BETWEEN_NAME_VALUE_PAIRS, fontMetrics.getHeight()
        ...
        constraints.anchor = GridBagConstraints.CENTER;
        panel.add(
                FastcoinWalletTitledPanel.createStent(FastcoinWalletTitledPanel.SEPARATION_BETWEEN_NAME_VALUE_PAIRS, separatorSize),
                constraints);
        ...
        constraints.gridheight = 1;
        constraints.anchor = GridBagConstraints.CENTER;
        panel.add(FastcoinWalletTitledPanel.createStent(fontMetrics.stringWidth(FastcoinWalletFrame.EXAMPLE_LONG_FIELD_TEXT), separatorSize),
                constraints);
        ...
        constraints.gridheight = 1;
        constraints.anchor = GridBagConstraints.CENTER;
        panel.add(FastcoinWalletTitledPanel.createStent(separatorSize), constraints);
        ...
        constraints.anchor = GridBagConstraints.LINE_START;
        JPanel dashedStent = FastcoinWalletTitledPanel.createStent(smallSeparatorSize);
    ...
    public void displaySidePanel() {
        ...
        if (showSidePanel) {
            ...
            sidePanelButton.setToolTipText(HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("sendFastcoinPanel.showLess.tooltip")));
            sidePanelButton.setMnemonic(mnemonicUtil.getMnemonic(controller.getLocaliser().getString(
                    "sendFastcoinPanel.showLess.mnemonic")));
            ...
            sidePanelButton.setText("");
            sidePanelButton.setBorderPainted(false);
            sidePanelButton.setToolTipText(HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("sendFastcoinPanel.showMore.tooltip")));
            sidePanelButton.setMnemonic(mnemonicUtil.getMnemonic(controller.getLocaliser().getString(
                    "sendFastcoinPanel.showMore.mnemonic")));
            sidePanelButton.setVerticalTextPosition(JLabel.BOTTOM);
            sidePanelButton.setHorizontalTextPosition(JLabel.LEFT);
        }
    }
    ...
    protected JPanel createAddressesHeaderPanel() {
        ...
        constraints.weighty = 0.01;
        constraints.anchor = GridBagConstraints.LINE_START;
        addressesHeaderPanel.add(FastcoinWalletTitledPanel.createStent(HELP_BUTTON_INDENT), constraints);

        createNewAddressAction = getCreateNewAddressAction();
        createNewButton = new FastcoinWalletButton(createNewAddressAction, controller);
        ...
        if (isReceiveFastcoin()) {
            // Put in a stent
            FastcoinWalletButton dummyDeleteButton = new FastcoinWalletButton(deleteAddressAction, controller);
            JPanel deleteButtonStent = FastcoinWalletTitledPanel.createStent(dummyDeleteButton.getPreferredSize().width, dummyDeleteButton.getPreferredSize().height);
            offset = 1;
            constraints.fill = GridBagConstraints.NONE;
            constraints.gridx = 2;
            constraints.gridy = 0;
            ...
            deleteButton = new FastcoinWalletButton(deleteAddressAction, controller);
            offset = 1;
            constraints.fill = GridBagConstraints.NONE;
            constraints.gridx = 2;
            constraints.gridy = 0;
        ...
        constraints.weighty = 1;
        constraints.anchor = GridBagConstraints.CENTER;
        addressesHeaderPanel.add(FastcoinWalletTitledPanel.createStent(HELP_BUTTON_INDENT * 2), constraints);

        titleLabel = new JLabel();
        titleLabel.setHorizontalTextPosition(JLabel.CENTER);
        titleLabel.setText(getLocalisationString(ADDRESSES_TITLE, null));
        titleLabel.setFont(FontSizer.INSTANCE.getAdjustedDefaultFontWithDelta(ColorAndFontConstants.FASTCOIN_WALLET_LARGE_FONT_INCREASE));
    ...
    protected JPanel createAddressesPanel() {
        ...
        addressesTableModel = new AddressBookTableModel(this.fastcoinController, isReceiveFastcoin());
            ...
            if (enabled) {
                ...
                if (viewRow >= 0) {
                    ...
                    WalletAddressBookData rowData = addressesTableModel.getAddressBookDataByRow(selectedAddressRowModel,
                            thisAbstractTradePanel.isReceiveFastcoin());
                    if (rowData != null) {
                        fastcoinController.getModel().setActiveWalletPreference(thisAbstractTradePanel.getAddressConstant(),
                                rowData.getAddress());
                        fastcoinController.getModel().setActiveWalletPreference(thisAbstractTradePanel.getLabelConstant(),
                                rowData.getLabel());
                        ...
                        if (parsedAmountFST != null && parsedAmountFST.getAmount() != null) {
                            amountForQRCode = controller.getLocaliser().fastcoinValueToStringNotLocalised(parsedAmountFST.getAmount().toBigInteger(), false, false);
                        }
    ...
    static class LeftJustifiedRenderer extends DefaultTableCellRenderer {
        ...
        FastcoinWalletLabel label = new FastcoinWalletLabel("");
    ...
    protected void createQRCodePanel(JPanel panel, GridBagConstraints constraints) {
        qrCodeLabel = new FastcoinWalletLabel("", JLabel.CENTER);
        ...
        JPanel verticalStent = FastcoinWalletTitledPanel.createStent(1, QRCODE_HEIGHT);
    ...
    private void setDragLabelTextAndTooltip() {
        if (!isReceiveFastcoin()) {
            ...
            if (icon == null || icon.getIconHeight() == -1) {
                qrCodeLabel.setText(createCenteredMultilineLabelText(controller.getLocaliser().getString("sendFastcoinPanel.dragFastcoinLabelQRcode.text")));
            } else {
                qrCodeLabel.setText("");                
            }
            qrCodeLabel.setToolTipText(HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("sendFastcoinPanel.dragFastcoinLabelQRcode.tooltip")));
        }
    }
    ...
    protected void updateQRCodePanel() {
        ...
        if (amountFSTTextField != null) {
            CurrencyConverterResult converterResult = CurrencyConverter.INSTANCE.parseToFST(amountFSTTextField.getText());
            if (converterResult.isFstMoneyValid()) {
                amount = controller.getLocaliser().fastcoinValueToStringNotLocalised(parsedAmountFST.getAmount().toBigInteger(), false, false);
            }
        }
        String label = "";
    ...
    protected void createQRCodeButtonPanel(JPanel panel, GridBagConstraints constraints) {
        ...
        copyQRCodeImageButton = new FastcoinWalletButton(copyQRCodeImageAction, controller);

        PasteSwatchAction pasteSwatchAction = new PasteSwatchAction(this.fastcoinController, this,
                ImageLoader.createImageIcon(ImageLoader.PASTE_ICON_FILE));
        pasteSwatchButton = new FastcoinWalletButton(pasteSwatchAction, controller);

        qrCodeButtonPanelStent1 = FastcoinWalletTitledPanel.createStent(smallSeparatorSize);
        qrCodeButtonPanelStent2 = FastcoinWalletTitledPanel.createStent(smallSeparatorSize);
        qrCodeButtonPanelStent3 = FastcoinWalletTitledPanel.createStent(smallSeparatorSize);

        ZoomAction zoomAction = new ZoomAction(this.fastcoinController, ImageLoader.createImageIcon(ImageLoader.ZOOM_ICON_FILE), mainFrame, this);
        zoomButton = new FastcoinWalletButton(zoomAction, controller);
        zoomButton.setText("");

        qrCodeButtonPanelStent4 = FastcoinWalletTitledPanel.createStent((int)zoomButton.getPreferredSize().getWidth(), (int)zoomButton.getPreferredSize().getHeight());
    ...
    protected JPanel createAmountPanel() {
        ...
        amountFSTTextField = new FastcoinWalletTextField("", 10, controller);
        amountFSTTextField.setHorizontalAlignment(JTextField.TRAILING);
        amountFSTTextField.setMinimumSize(new Dimension((int) (longFieldWidth * 0.45), getFontMetrics(
                FontSizer.INSTANCE.getAdjustedDefaultFont()).getHeight()
                + TEXTFIELD_VERTICAL_DELTA));
        amountFSTTextField.setPreferredSize(new Dimension((int) (longFieldWidth * 0.45), getFontMetrics(
                FontSizer.INSTANCE.getAdjustedDefaultFont()).getHeight()
                + TEXTFIELD_VERTICAL_DELTA));
        amountFSTTextField.setMaximumSize(new Dimension((int) (longFieldWidth * 0.45), getFontMetrics(
                FontSizer.INSTANCE.getAdjustedDefaultFont()).getHeight()
                + TEXTFIELD_VERTICAL_DELTA));
        amountFSTTextField.addKeyListener(new AmountFSTKeyListener());
        ...
        constraints2.gridheight = 1;
        constraints2.anchor = GridBagConstraints.LINE_START;
        amountPanel.add(amountFSTTextField, constraints2);
        ...
        constraints2.gridheight = 1;
        constraints2.anchor = GridBagConstraints.CENTER;
        amountPanel.add(FastcoinWalletTitledPanel.createStent(3), constraints2);

        FastcoinWalletLabel amountUnitFSTLabel = new FastcoinWalletLabel(controller.getLocaliser().getString("sendFastcoinPanel.amountUnitLabel"));
        amountUnitFSTLabel.setHorizontalTextPosition(SwingConstants.LEADING);
        amountUnitFSTLabel.setToolTipText(HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("sendFastcoinPanel.amountUnitLabel.tooltip")));
        constraints2.fill = GridBagConstraints.NONE;
        constraints2.gridx = 2;
        constraints2.gridy = 0;
        constraints2.weightx = 0.1;
        ...
        constraints2.gridheight = 1;
        constraints2.anchor = GridBagConstraints.CENTER;
        amountPanel.add(amountUnitFSTLabel, constraints2);

        amountEqualsLabel = new FastcoinWalletLabel("   =   "); // 3 spaces either side
        ...
        amountPanel.add(FastcoinWalletTitledPanel.createStent(amountEqualsLabel.getPreferredSize().width, amountEqualsLabel.getPreferredSize().height), constraints2);

        amountFiatTextField = new FastcoinWalletTextField("", 8, controller);
        ...
        amountPanel.add(FastcoinWalletTitledPanel.createStent(amountFiatTextField.getPreferredSize().width, amountFiatTextField.getPreferredSize().height), constraints2);
        ...
        amountUnitFiatLabel = new FastcoinWalletLabel("");
        ...
        amountPanel.add(FastcoinWalletTitledPanel.createStent(amountUnitFiatLabel.getPreferredSize().width, amountUnitFiatLabel.getPreferredSize().height), constraints2);
...
//                deleteButton.setEnabled(deleteEnable);
    ...
    protected class QRCodeKeyListener implements KeyListener {
        ...
        public void keyReleased(KeyEvent e) {
            ...
            WalletInfoData walletInfo = fastcoinController.getModel().getActiveWalletInfo();
            if (walletInfo == null) {
                walletInfo = new WalletInfoData(fastcoinController.getModel().getActiveWalletFilename(), fastcoinController.getModel().getActiveWallet(), FastcoinWalletVersion.PROTOBUF_ENCRYPTED);
                fastcoinController.getModel().setActiveWalletInfo(walletInfo);
            }
            address = WhitespaceTrimmer.trim(address);
            addressesTableModel.setAddressBookDataByRow(addressBookData, selectedAddressRowModel, isReceiveFastcoin());

            selectRowInTableFromModelRow(selectedAddressRowModel);

            fastcoinController.getModel().setActiveWalletPreference(thisAbstractTradePanel.getAddressConstant(), address);
            fastcoinController.getModel().setActiveWalletPreference(thisAbstractTradePanel.getLabelConstant(), label);
            fastcoinController.getModel().setActiveWalletPreference(thisAbstractTradePanel.getAmountConstant(), amount);
            fastcoinController.getModel().getActivePerWalletModelData().setDirty(true);
    ...
    protected class AmountFSTKeyListener implements KeyListener {
        /** Handle the key typed event in the amount FST field */
        @Override
        public void keyTyped(KeyEvent e) {
        }

        /** Handle the key-pressed event in the amount FST field */
        @Override
        public void keyPressed(KeyEvent e) {
            // do nothing
        }

        /** Handle the key-released event in the amount FST field */
        @Override
        public void keyReleased(KeyEvent e) {
            String address = null;
            if (addressTextField != null) {
    ...
    protected class AmountFiatKeyListener implements KeyListener {
        ...
        public void keyReleased(KeyEvent e) {
            ...
            String amountFSTAsString = updateFSTAmount(amountFiat);

            displayQRCode(address, amountFSTAsString, label);
        }
    }
    
    protected void updateFiatAmount() {
        // Convert the FST into fiat and populate the fiat amount label.
        if (CurrencyConverter.INSTANCE.getRate() != null && CurrencyConverter.INSTANCE.isShowingFiat()) {
            try {
                if (parsedAmountFST != null) {
                    parsedAmountFiat = CurrencyConverter.INSTANCE.convertFromFSTToFiat(parsedAmountFST.getAmount().toBigInteger());
    ...
    protected String updateFSTAmount(String amountFiat) {
        CurrencyConverterResult converterResult = CurrencyConverter.INSTANCE.convertFromFiatToFST(amountFiat);
        if (converterResult.isFiatMoneyValid() && converterResult.isFstMoneyValid()) {
            parsedAmountFST = converterResult.getFstMoney();
            String amountFSTAsString = controller.getLocaliser().fastcoinValueToStringNotLocalised(parsedAmountFST.getAmount().toBigInteger(), false,
                    false);
            fastcoinController.getModel().setActiveWalletPreference(thisAbstractTradePanel.getAmountConstant(), amountFSTAsString);
            fastcoinController.getModel().getActivePerWalletModelData().setDirty(true);

            amountFSTTextField.setText(CurrencyConverter.INSTANCE.getFSTAsLocalisedString(parsedAmountFST));
            if (notificationLabel != null) {
                notificationLabel.setText("");
            }
            return amountFSTAsString;
        } else {
            parsedAmountFST = null;
            ...
            if (notificationLabel != null) {
                ...
                if (!converterResult.isFstMoneyValid() && converterResult.getFstMessage() != null) {
                    if (message.length() > 0) {
                        message = message + ". ";
                    }
                    message = message + converterResult.getFstMessage();
                }
    ...
    public void displayQRCode(String address, String amount, String label) {
        if (qrCodeGenerator == null) {
            qrCodeGenerator = new QRCodeGenerator(this.fastcoinController);
        }
        try {
    ...
    public boolean processDecodedString(String decodedString, ImageIcon icon) {
        ...
        if (haveFilesChanged) {
            ...
            // Early MultiBitWallet versions did not URL encode the label hence may
            // have illegal embedded spaces - convert to ENCODED_SPACE_CHARACTER
            // i.e be lenient
            String uriString = decodedString.replace(" ", FastcoinController.ENCODED_SPACE_CHARACTER);
            FastcoinURI fastcoinURI;
            try {
                fastcoinURI = new FastcoinURI(this.fastcoinController.getModel().getNetworkParameters(), uriString);
            ...
            Address address = fastcoinURI.getAddress();
            ...
            if (amountFSTTextField != null) {
                ...
                if (converterResult.isFstMoneyValid()) {
                    parsedAmountFST = converterResult.getFstMoney();
                    amountString = controller.getLocaliser().fastcoinValueToStringNotLocalised(parsedAmountFST.getAmount().toBigInteger(), false, false);
                    amountStringLocalised = CurrencyConverter.INSTANCE.getFSTAsLocalisedString(parsedAmountFST);
                } else {
                    parsedAmountFST = null;
                    if (notificationLabel != null) {
                        notificationLabel.setText(converterResult.getFstMessage());
            ...
            if (fastcoinURI.getAmount() != null) {
                amountString = controller.getLocaliser().fastcoinValueToStringNotLocalised(fastcoinURI.getAmount(), false, false);
                parsedAmountFST = Money.of(CurrencyConverter.INSTANCE.FASTCOIN_CURRENCY_UNIT, new BigDecimal(fastcoinURI.getAmount()));
                amountStringLocalised = CurrencyConverter.INSTANCE.getFSTAsLocalisedString(parsedAmountFST);
            }
            log.debug("AbstractTradePanel - ping 4");
            String decodedLabel = "";
            try {
                if (fastcoinURI.getLabel() != null) {
                    decodedLabel = java.net.URLDecoder.decode(fastcoinURI.getLabel(), "UTF-8");
                }
            ...
            if (rowToSelectModel >= 0) {
                ...
                this.fastcoinController.getModel().getActiveWalletInfo().addSendingAddress(addressBookData);
                this.fastcoinController.getModel().getActivePerWalletModelData().setDirty(true);
            ...
            this.fastcoinController.getModel().setActiveWalletPreference(FastcoinModel.SEND_ADDRESS, addressString);
            log.debug("AbstractTradePanel - ping 8");
            this.fastcoinController.getModel().setActiveWalletPreference(FastcoinModel.SEND_LABEL, decodedLabel);
            log.debug("AbstractTradePanel - ping 9");

            this.fastcoinController.getModel().setActiveWalletPreference(FastcoinModel.SEND_AMOUNT, amountString);
            log.debug("AbstractTradePanel - ping 10");
            addressTextField.setText(addressString);
            log.debug("AbstractTradePanel - ping 11");
            amountFSTTextField.setText(amountStringLocalised);
    ...
    public void selectRows() {
        ...
        String address = this.fastcoinController.getModel().getActiveWalletPreference(getAddressConstant());
        String amount = "";
        if (amountFSTTextField != null) {
            CurrencyConverterResult converterResult = CurrencyConverter.INSTANCE.parseToFST(amountFSTTextField.getText());
        ...
        int rowToSelectModel = addressesTableModel.findRowByAddress(address, isReceiveFastcoin());
        if (rowToSelectModel >= 0) {
            selectedAddressRowModel = rowToSelectModel;
    ...
    public String getAmount() {
        ...
        if (amountFSTTextField != null) {
            ...
            if (converterResult.isFstMoneyValid()) {
                amount = controller.getLocaliser().fastcoinValueToStringNotLocalised(converterResult.getFstMoney().getAmount().toBigInteger(), false, false);
            }
        }
       return amount;
    }
    ...
    public FastcoinWalletTextField getAddressTextField() {
