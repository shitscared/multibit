        ...
        private String getUnconfirmedConfidenceToolTip(Transaction transaction) {
            ...
            if (transaction != null) {
                if (transaction.getLockTime() > 0) {
                    ...
                    transactionTrustfulness = MultiBitWallet.getController().getLocaliser().getString("fastcoinWalletFrame.status.notConfirmedAndNotStandard") + ".";
                } else {
                    // Normal transaction.
                    if (transaction.isCoinBase()) {
                        transactionTrustfulness = MultiBitWallet.getController().getLocaliser().getString("fastcoinWalletFrame.status.notConfirmedAndCoinbase") + ".";
                    } else {
                        transactionTrustfulness = MultiBitWallet.getController().getLocaliser().getString("fastcoinWalletFrame.status.notConfirmed") + ".";
                    }
                }
            }
