...
import org.multibit.viewsystem.swing.FastcoinWalletTabbedPane;
...
public class WalletListPanel extends JPanel implements Viewable, WalletBusyListener, ComponentListener, CurrencyConverterListener {
    ...
    private FastcoinWalletTabbedPane tabbedPane;
     ...
     */
    public WalletListPanel(FastcoinController fastcoinController, FastcoinWalletFrame mainFrame) {
    ...
    private void displayView(DisplayHint displayHint, boolean blinkEnabled) {
        if (walletPanels != null) {
            synchronized(walletPanels) {
                ...
                for (SingleWalletPanel loopSingleWalletPanel : walletPanels) {
                    ...
                    boolean modelBlinkEnabled = this.fastcoinController.getModel().isBlinkEnabled();
        ...
        String activeWalletFilename = this.fastcoinController.getModel().getActiveWalletFilename();
        WalletData activePerModelData = this.fastcoinController.getModel().getPerWalletModelDataByWalletFilename(activeWalletFilename);
    ...
    public void initUI() {
        ...
        tabbedPane = new FastcoinWalletTabbedPane(controller);
            ...
            synchronized (walletPanels) {
                for (WalletData loopPerWalletModelData : perWalletModelDataList) {
                    if (loopPerWalletModelData.getWallet() != null) {
                        ...
                        SingleWalletPanel loopPanel = new SingleWalletPanel(loopPerWalletModelData, this.fastcoinController, mainFrame, this);
    ...
    private JPanel createButtonPanel() {
        ...
        CreateWalletSubmitAction createNewWalletAction = new CreateWalletSubmitAction(this.fastcoinController, ImageLoader.createImageIcon(ImageLoader.CREATE_NEW_ICON_FILE), mainFrame);
        FastcoinWalletButton createNewWalletButton = new FastcoinWalletButton(createNewWalletAction, controller);
        ...
        if (walletPanels != null) {
            synchronized (walletPanels) {
                ...
                for (SingleWalletPanel loopSingleWalletPanel : walletPanels) {
                    if (loopSingleWalletPanel.getPerWalletModelData().getWalletFilename() != null) {
                        if (loopSingleWalletPanel.getPerWalletModelData().getWalletFilename()
                                .equals(this.fastcoinController.getModel().getActiveWalletFilename())) {
                ...
                if (nextSelectedWalletIndex > -1) {
                    this.fastcoinController.getModel().setActiveWalletByFilename(walletPanels.get(nextSelectedWalletIndex).getPerWalletModelData().getWalletFilename());
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (selectedWalletPanel != null) {
                ...
                if (!selectedWalletPanel.getPerWalletModelData().getWalletFilename()
                        .equals(fastcoinController.getModel().getActiveWalletFilename())) {
                    fastcoinController.getModel()
    ...
    public void componentResized(ComponentEvent arg0) {
        ...
        if (scrollPane.getVerticalScrollBar().isVisible()) {
            preferredWalletWidth -= FastcoinWalletFrame.SCROLL_BAR_DELTA;
        }         
