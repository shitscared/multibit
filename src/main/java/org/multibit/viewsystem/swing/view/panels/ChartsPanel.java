...
public class ChartsPanel extends JPanel implements Viewable, ComponentListener {
  ...
  private static final double NUMBER_OF_SATOSHI_IN_ONE_FST = 100000000;
   ...
   */
    public ChartsPanel(FastcoinController fastcoinController, FastcoinWalletFrame mainFrame) {
  ...
  private JPanel createChartPanel() {
    try {
        ...
        if (chartDataCollection == null || chartDataCollection.size() == 0) {
          ...
          for (ChartData chartData : chartDataCollection) {
            if (chartData != null && chartData.getDate() != null && chartData.getValue() != null) {
              ...
              yData.add(chartData.getValue().doubleValue() / NUMBER_OF_SATOSHI_IN_ONE_FST);
            }
          }
        }
      }
      ...
      String currencyUnitSuffix = " (" + controller.getLocaliser().getString("sendFastcoinPanel.amountUnitLabel") + ")";
      String balanceLabel = controller.getLocaliser().getString("fastcoinWalletFrame.balanceLabel") + currencyUnitSuffix;
  ...
  private Collection<ChartData> getChartData() {
    ...
    if (controller.getModel() == null || this.fastcoinController.getModel().getActiveWallet() == null) {
      return new ArrayList<ChartData>();
    }

    ArrayList<Transaction> allTransactions = new ArrayList<Transaction>(this.fastcoinController.getModel().getActiveWallet().getTransactions(false));
    ...
    try {
      ...
      if (allTransactions == null || allTransactions.size() == 0) {
        ...
        for (Transaction loop : allTransactions) {
          balance = balance.add(loop.getValue(this.fastcoinController.getModel().getActiveWallet()));
    ...
    } catch (com.google.fastcoin.core.ScriptException e1) {
      e1.printStackTrace();
    }
