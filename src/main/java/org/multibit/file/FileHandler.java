...
import org.multibit.network.FastcoinWalletService;
import org.wallet.store.FastcoinWalletProtobufSerializer;
...
/**
 * Class consolidating the File IO in MultiBitWallet for wallets and wallet infos.
 * 
 * @author jim
 * 
 */
public class FileHandler {
    ...
    public static final String USER_PROPERTIES_FILE_NAME = "fastcoin-wallet.properties";
    public static final String USER_PROPERTIES_HEADER_TEXT = "wallet";
    ...
    private FastcoinWalletProtobufSerializer walletProtobufSerializer;
    ...
    public FileHandler(FastcoinController fastcoinController) {
        this.fastcoinController = fastcoinController;
        this.controller = this.fastcoinController;

        walletProtobufSerializer = new FastcoinWalletProtobufSerializer();
    }
    ...
    public WalletData loadFromFile(File walletFile) throws WalletLoadException, WalletVersionException {
        ...
        try {
            ...
            if (isWalletSerialised(walletFile)) {
                ...
                walletInfo = new WalletInfoData(walletFilenameToUseInModel, null, FastcoinWalletVersion.PROTOBUF_ENCRYPTED);
            }
             ...
             if (!walletWasLoadedSuccessfully) {
                 ...
                 while (!walletWasLoadedSuccessfully && iterator.hasNext()) {
                    ...
                    try {
                        ...
                        MessageManager.INSTANCE.addMessage(new Message(fastcoinController.getLocaliser().getString("fileHandler.walletCannotLoadUsingBackup",
            ...
            if (walletWasLoadedSuccessfully) {
                ...
                wallet.setNetworkParameters(fastcoinController.getModel().getNetworkParameters());

                perWalletModelData = fastcoinController.getModel().addWallet(this.fastcoinController, wallet,
                ...
                if (useBackupWallets) {
                    ...
                    walletInfo.put(FastcoinModel.WALLET_BACKUP_FILE, "");
                ...
                // Report failure to user.
                String messageText = fastcoinController.getLocaliser().getString("fileHandler.unableToLoadWalletOrBackups", new String[] {walletFilenameToUseInModel});
                if (!errorMessages.isEmpty()) {
                    ...
                    messageText = messageText + "\n" + fastcoinController.getLocaliser().getString("deleteWalletConfirmDialog.walletDeleteError2", new String[]{errorMessagesAsString.toString()});
                }
    ...
    public void saveWalletAndWalletInfoSimple(WalletData perWalletModelData, String walletFilename, String walletInfoFilename) {
        ...
        try {
            if (perWalletModelData.getWallet() != null) {
                // MultiBitWallet description is currently stored in the wallet info
                ...
                if (FastcoinWalletVersion.SERIALIZED == walletInfo.getWalletVersion()) {
                    ...
                    if (walletIsActuallyEncrypted) {
                        walletInfo.setWalletVersion(FastcoinWalletVersion.PROTOBUF_ENCRYPTED);
                    }

                    if (FastcoinWalletVersion.PROTOBUF == walletInfo.getWalletVersion()) {
                        // Save as a MultiBitWallet message.
                        perWalletModelData.getWallet().saveToFile(walletFile);
                    } else if (FastcoinWalletVersion.PROTOBUF_ENCRYPTED == walletInfo.getWalletVersion()) {
                        fileOutputStream = new FileOutputStream(walletFile);

                        // Save as a MultiBitWallet message with a mandatory extension
                        // to prevent loading by older versions of wallet.
                        ...
                        throw new WalletVersionException("Cannot save wallet '" + perWalletModelData.getWalletFilename()
                                ...
                                + "' but this version of MultiBitWallet does not understand that format.");
                    }
                }
    ...
    private void saveWalletAndWalletInfo(WalletData perWalletModelData, String walletFilename, String walletInfoFilename) {
        ...
        try {
            if (perWalletModelData.getWallet() != null) {
                ...
                String oldBackupFilename = perWalletModelData.getWalletInfo().getProperty(FastcoinModel.WALLET_BACKUP_FILE);
                ...
                if (FastcoinWalletVersion.PROTOBUF == walletInfo.getWalletVersion()
                        || FastcoinWalletVersion.PROTOBUF_ENCRYPTED == walletInfo.getWalletVersion()) {
                ...
                if (FastcoinWalletVersion.PROTOBUF == walletInfo.getWalletVersion()
                        || FastcoinWalletVersion.PROTOBUF_ENCRYPTED == walletInfo.getWalletVersion()) {
                    perWalletModelData.getWalletInfo().put(FastcoinModel.WALLET_BACKUP_FILE, newBackupFilename);
    ...
    public File backupPrivateKeys(CharSequence passwordToUse) throws IOException, KeyCrypterException {
        ...
        if (passwordToUse != null && passwordToUse.length() > 0) {
            if (controller.getModel() != null
                    && this.fastcoinController.getModel().getActiveWalletInfo() != null
                    && this.fastcoinController.getModel().getActiveWalletInfo().getWalletVersion() == FastcoinWalletVersion.PROTOBUF_ENCRYPTED) {
                // Save a backup copy of the private keys, encrypted with the passwordToUse.
                PrivateKeysHandler privateKeysHandler = new PrivateKeysHandler(this.fastcoinController.getModel()
                        .getNetworkParameters());
                String privateKeysBackupFilename = BackupManager.INSTANCE.createBackupFilename(new File(this.fastcoinController.getModel()
                        .getActiveWalletFilename()), BackupManager.PRIVATE_KEY_BACKUP_DIRECTORY_NAME, false, false, FastcoinModel.PRIVATE_KEY_FILE_EXTENSION);
                privateKeysBackupFile = new File(privateKeysBackupFilename);
                BlockChain blockChain = null;
                if (this.fastcoinController.getFastcoinWalletService() != null) {
                    blockChain = this.fastcoinController.getFastcoinWalletService().getChain();
                }

                privateKeysHandler.exportPrivateKeys(privateKeysBackupFile, this.fastcoinController.getModel().getActiveWallet(),
                        blockChain, true, passwordToUse, passwordToUse);
            } else {
                log.debug("MultiBitWallet '" + this.fastcoinController.getModel().getActiveWalletFilename()
    ...
    private String copyExistingWalletToBackupAndDeleteOriginal(File walletFile) throws IOException {
        String newWalletBackupFilename = BackupManager.INSTANCE.createBackupFilename(walletFile, BackupManager.ROLLING_WALLET_BACKUP_DIRECTORY_NAME, false, false, FastcoinModel.WALLET_FILE_EXTENSION);
    ...
    public boolean haveFilesChanged(WalletData perWalletModelData) {
        ...
        if (walletInfo != null) {
            synchronized (walletInfo) {
                ...
                if (!walletFileSize.equals(walletInfo.getProperty(FastcoinModel.WALLET_FILE_SIZE))) {
                    haveFilesChanged = true;
                }

                if (!walletFileLastModified.equals(walletInfo.getProperty(FastcoinModel.WALLET_FILE_LAST_MODIFIED))) {
                    haveFilesChanged = true;
                }

                if (!walletInfoFileSize.equals(walletInfo.getProperty(FastcoinModel.WALLET_INFO_FILE_SIZE))) {
                    haveFilesChanged = true;
                }
                ...
                if (haveFilesChanged) {
                    ...
                    log.debug(FastcoinModel.WALLET_FILE_SIZE + " " + walletFileSize + " ," + FastcoinModel.WALLET_FILE_LAST_MODIFIED
                            + " " + walletFileLastModified + " ," + FastcoinModel.WALLET_INFO_FILE_SIZE + " " + walletInfoFileSize
                            + " ," + FastcoinModel.WALLET_INFO_FILE_LAST_MODIFIED + " " + walletInfoFileLastModified);
                }
            }
        }
    ...
    private void rememberFileSizesAndLastModified(File walletFile, WalletInfoData walletInfo) {
        ...
        walletInfo.put(FastcoinModel.WALLET_FILE_SIZE, "" + walletFileSize);
        walletInfo.put(FastcoinModel.WALLET_FILE_LAST_MODIFIED, "" + walletFileLastModified);
        walletInfo.put(FastcoinModel.WALLET_INFO_FILE_SIZE, "" + walletInfoFileSize);
        walletInfo.put(FastcoinModel.WALLET_INFO_FILE_LAST_MODIFIED, "" + walletInfoFileLastModified);

        log.debug("rememberFileSizesAndLastModified: MultiBitWallet filename " + walletFilename + " , " + FastcoinModel.WALLET_FILE_SIZE
                + " " + walletFileSize + " ," + FastcoinModel.WALLET_FILE_LAST_MODIFIED + " " + walletFileLastModified + " ,"
                + FastcoinModel.WALLET_INFO_FILE_SIZE + " " + walletInfoFileSize + " ,"
                + FastcoinModel.WALLET_INFO_FILE_LAST_MODIFIED + " " + walletInfoFileLastModified);
    }

    public static void writeUserPreferences(FastcoinController fastcoinController) {
        final Controller controller = fastcoinController;
        // Save all the wallets' filenames in the user preferences.
        if (fastcoinController.getModel().getPerWalletModelDataList() != null) {
            ...
            for (WalletData perWalletModelData : perWalletModelDataList) {
                ...
                if (perWalletModelData.getWalletInfo().getWalletVersion() == FastcoinWalletVersion.PROTOBUF_ENCRYPTED) {
                    if (!protobuf3List.contains(perWalletModelData.getWalletFilename())) {
                        ...
                        || perWalletModelData.getWalletInfo().getWalletVersion() == FastcoinWalletVersion.SERIALIZED
                        || perWalletModelData.getWalletInfo().getWalletVersion() == FastcoinWalletVersion.PROTOBUF) {
            ...
            for (String walletFilename : orderList) {
                controller.getModel().setUserPreference(FastcoinModel.WALLET_ORDER_PREFIX + orderCount, walletFilename);
                orderCount++;
            }
            controller.getModel().setUserPreference(FastcoinModel.WALLET_ORDER_TOTAL, "" + orderList.size());

            int earlyCount = 1;
            for (String walletFilename : earlyList) {
                controller.getModel().setUserPreference(FastcoinModel.EARLY_WALLET_FILENAME_PREFIX + earlyCount, walletFilename);
                earlyCount++;
            }
            controller.getModel().setUserPreference(FastcoinModel.NUMBER_OF_EARLY_WALLETS, "" + earlyList.size());

            int protobuf3Count = 1;
            for (String walletFilename : protobuf3List) {
                controller.getModel().setUserPreference(FastcoinModel.PROTOBUF3_WALLET_FILENAME_PREFIX + protobuf3Count,
                        walletFilename);
                protobuf3Count++;
            }
            controller.getModel().setUserPreference(FastcoinModel.NUMBER_OF_PROTOBUF3_WALLETS, "" + protobuf3List.size());
            controller.getModel().setUserPreference(FastcoinModel.ACTIVE_WALLET_FILENAME,
                    fastcoinController.getModel().getActiveWalletFilename());
        }
     ...
     * the user's application data directory when MultiBitWallet is first used.
     * 
    ...
    public void copyCheckpointsFromInstallationDirectory(String destinationCheckpointsFilename) throws IOException {
        ...
        if (!destinationCheckpoints.exists()) {
            ...
            String filePrefix = FastcoinWalletService.getFilePrefix();
            String checkpointsFilename = filePrefix + FastcoinWalletService.CHECKPOINTS_SUFFIX;
