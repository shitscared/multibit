 ...
 * the License.
 */
package org.multibit.controller.fastcoin;
...
/**
 * The MVC controller for MultiBitWallet.
 * 
 * @author jim
 */
public class FastcoinController extends AbstractController<CoreController> implements WalletEventListener, TransactionConfidence.Listener {

    public static final String ENCODED_SPACE_CHARACTER = "%20";
    private Logger log = LoggerFactory.getLogger(FastcoinController.class);
    ...
    /**
     * The fastcoinj network interface.
     */
    private FastcoinWalletService fastcoinWalletService;
    /**
     ...
     */
    private FastcoinModel model;
    
    /**
     * Used for testing only.
     */
    public FastcoinController(CoreController coreController) {
        super(coreController);

        this.walletBusyListeners = new ArrayList<WalletBusyListener>();
        this.fileHandler = new FileHandler(this);
        this.eventHandler = new EventHandler(this);
        this.peerEventListener = new FastcoinPeerEventListener(this);
        
        this.addEventHandler(this.getEventHandler());
    }
    
    @Override
    public FastcoinModel getModel() {
        return model;
    }

    public void setModel(FastcoinModel model) {
        this.model = model;
    }
    ...
    public WalletData addWalletFromFilename(String walletFilename) throws IOException {
        ...
        if (fastcoinWalletService != null) {
            perWalletModelDataToReturn = fastcoinWalletService.addWalletFromFilename(walletFilename);
        }
    ...
    public FastcoinWalletService getFastcoinWalletService() {
        return fastcoinWalletService;
    }

    public void setFastcoinWalletService(FastcoinWalletService fastcoinWalletService) {
        this.fastcoinWalletService = fastcoinWalletService;
    }
    ...
    public synchronized void handleOpenURI() {
        log.debug("handleOpenURI.1 called and rawFastcoinURI ='" + eventHandler.rawFastcoinURI + "'");
        if (eventHandler.rawFastcoinURI != null) {
            handleOpenURI(eventHandler.rawFastcoinURI.toString());
        }
    }

    public synchronized void handleOpenURI(String rawFastcoinURIString) {
        log.debug("handleOpenURI.2 called and rawFastcoinURIString ='" + rawFastcoinURIString + "'");

        // get the open URI configuration information
        String showOpenUriDialogText = getModel().getUserPreference(FastcoinModel.OPEN_URI_SHOW_DIALOG);
        String useUriText = getModel().getUserPreference(FastcoinModel.OPEN_URI_USE_URI);
        ...
        if (Boolean.FALSE.toString().equalsIgnoreCase(useUriText)
            ...
            log.debug("Fastcoin URI ignored because useUriText = '" + useUriText + "', showOpenUriDialogText = '"
        ...
        if (rawFastcoinURIString == null || rawFastcoinURIString.equals("")) {
            log.debug("No Fastcoin URI found to handle");
            return;
        }
        // Process the URI
        ...
        // be lenient
        String uriString = rawFastcoinURIString.replace(" ", ENCODED_SPACE_CHARACTER);
        FastcoinURI fastcoinURI;
        try {
            fastcoinURI = new FastcoinURI(getModel().getNetworkParameters(), uriString);
        ...
        String address = fastcoinURI.getAddress().toString();
        String label = "";
        try {
            // No label? Set it to a blank String otherwise perform a URL decode
            // on it just to be sure.
            label = null == fastcoinURI.getLabel() ? "" : URLDecoder.decode(fastcoinURI.getLabel(), "UTF-8");
        } catch (UnsupportedEncodingException e) {
            log.error("Could not decode the label in UTF-8. Unusual URI entry or platform.");
        }
        // No amount? Set it to zero.
        BigInteger numericAmount = null == fastcoinURI.getAmount() ? BigInteger.ZERO : fastcoinURI.getAmount();
        String amount = getLocaliser().fastcoinValueToStringNotLocalised(numericAmount, false, false);
        ...
        if (Boolean.FALSE.toString().equalsIgnoreCase(showOpenUriDialogText)) {
            ...
            getModel().setActiveWalletPreference(FastcoinModel.SEND_ADDRESS, address);
            getModel().setActiveWalletPreference(FastcoinModel.SEND_LABEL, label);
            getModel().setActiveWalletPreference(FastcoinModel.SEND_AMOUNT, amount);
            getModel().setActiveWalletPreference(FastcoinModel.SEND_PERFORM_PASTE_NOW, "true");
            log.debug("Routing straight to send view for address = " + address);

            getModel().setUserPreference(FastcoinModel.BRING_TO_FRONT, "true");
            displayView(View.SEND_FASTCOIN_VIEW);
            return;
        } else {
            // Show the confirm dialog to see if the user wants to use URI.
            // Populate the model with the URI data.
            getModel().setUserPreference(FastcoinModel.OPEN_URI_ADDRESS, address);
            getModel().setUserPreference(FastcoinModel.OPEN_URI_LABEL, label);
            getModel().setUserPreference(FastcoinModel.OPEN_URI_AMOUNT, amount);
    ...
    private class EventHandler extends AbstractEventHandler<FastcoinController> {

    /**
         * Multiple threads will write to this variable so require it to be
         * volatile to ensure that latest write is what gets read
     */
        private volatile URI rawFastcoinURI = null;

        public EventHandler(FastcoinController coreController) {
            super(coreController);
    }

        @Override
        public void handleOpenURIEvent(URI rawFastcoinURI) {
            this.rawFastcoinURI = rawFastcoinURI;
            handleOpenURI();
        ...
        public void handleQuitEvent(ExitAction exitAction) {
            exitAction.setFastcoinController(super.controller);
        }
    }
