...
public class Localiser {
    ...
    public static final String FASTCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY = "/i18n";
    public static final String FASTCOIN_WALLET_RESOURCE_BUNDLE_NAME = "viewer";
    ...
    public static final String LANGUAGE_PROPERTIES_FILENAME = FASTCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY + "/language.properties";
    ...
    public static final int NUMBER_OF_FRACTION_DIGITS_FOR_FASTCOIN = 8;
    ...
    public Localiser(Locale locale) {
        ...
        numberFormat.setMaximumFractionDigits(NUMBER_OF_FRACTION_DIGITS_FOR_FASTCOIN);
        numberFormatNotLocalised = NumberFormat.getInstance(Locale.ENGLISH);
        numberFormatNotLocalised.setMaximumFractionDigits(NUMBER_OF_FRACTION_DIGITS_FOR_FASTCOIN);
    ...
    public void setLocale(Locale locale) {
        String propertyFilename = FASTCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY + "/" + locale.getLanguage() + "/"
                + FASTCOIN_WALLET_RESOURCE_BUNDLE_NAME + PROPERTY_NAME_SUFFIX;
        String propertyFilenameBase = FASTCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY + "/" + FALLBACK_LANGUAGE_CODE + "/"
                + FASTCOIN_WALLET_RESOURCE_BUNDLE_NAME + PROPERTY_NAME_SUFFIX;

        if ("he".equals(locale.getLanguage()) || "iw".equals(locale.getLanguage())) {
            // Hebrew can be he or iw
            this.locale = new Locale("iw");
            propertyFilename = FASTCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY + "/he/" + FASTCOIN_WALLET_RESOURCE_BUNDLE_NAME + PROPERTY_NAME_SUFFIX;
        } if ("id".equals(locale.getLanguage()) || "in".equals(locale.getLanguage())) {
            // Indonesian can be id or in
            this.locale = new Locale("in");
            propertyFilename = FASTCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY + "/id/" + FASTCOIN_WALLET_RESOURCE_BUNDLE_NAME + PROPERTY_NAME_SUFFIX;
        } else {
            this.locale = locale;
        }
     ...
     **/
    public String fastcoinValueToString(BigInteger value, boolean addUnit, boolean blankZero) {
        ...
        BigDecimal valueInFST = new BigDecimal(value).divide(new BigDecimal(Utils.COIN));
        toReturn = toReturn + numberFormat.format(valueInFST.doubleValue());

        if (addUnit) {
            toReturn = toReturn + " " + getString("sendFastcoinPanel.amountUnitLabel");
        }
        return toReturn;     
    }
    /**
     ...
     **/
    public String fastcoinValueToStringNotLocalised(BigInteger value, boolean addUnit, boolean blankZero) {
        if (blankZero && value.compareTo(BigInteger.ZERO) == 0) {
            return "";
        }
        ...
        BigDecimal valueInFST = new BigDecimal(value).divide(new BigDecimal(Utils.COIN));
        toReturn = toReturn + numberFormatNotLocalised.format(valueInFST.doubleValue());
