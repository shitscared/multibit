...
/**
 * TimerTask to regularly check the 'health' of FastcoinWallet.
...
public class HealthCheckTimerTask extends TimerTask {
   ...
   *
   * @param fastcoinController Controller giving access to Fastcoin object model
   */
  public HealthCheckTimerTask(FastcoinController fastcoinController) {
    this.fastcoinController = fastcoinController;
    this.controller = fastcoinController;
      ...
      //if (counter == 0) {
        ...
        if (perWalletModelDataList != null) {
          ...
          while (iterator.hasNext()) {
            ...
            if (fastcoinController.getFileHandler() != null) {
              // See if the files have been changed by another
              // process (non FastcoinWallet).
              boolean haveFilesChanged = fastcoinController.getFileHandler().haveFilesChanged(loopModelData);
              if (haveFilesChanged) {
                ...
                if (!previousFilesHaveBeenChanged) {
                  ...
                  fastcoinController.fireFilesHaveBeenChangedByAnotherProcess(loopModelData);
              ...
              if (loopModelData.isDirty()) {
                ...
                try {
                  fastcoinController.getFileHandler().savePerWalletModelData(loopModelData, false);
                  log.debug("... done.");
...
//      if (fastcoinController.getFastcoinWalletService() != null && fastcoinController.getFastcoinWalletService().getPeerGroup() != null) {
//        PeerGroup peerGroup = fastcoinController.getFastcoinWalletService().getPeerGroup();
