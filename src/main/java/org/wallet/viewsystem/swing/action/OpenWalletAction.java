...
import org.wallet.network.FastcoinWalletCheckpointManager;
...
public class OpenWalletAction extends AbstractAction {
     ...
     */
    public OpenWalletAction(FastcoinController fastcoinController, ImageIcon icon, FastcoinWalletFrame mainFrame) {
        super(fastcoinController.getLocaliser().getString("openWalletAction.text"), icon);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            if (fileChooser == null) {
                ...
                if (controller.getModel() != null && this.fastcoinController.getModel().getActiveWalletFilename() != null) {
                ...
                if (file != null) {
                    if (!file.isDirectory()) {
                        ...
                        if (controller != null && controller.getModel() != null) {
                            List<WalletData> perWalletDataModels = this.fastcoinController.getModel().getPerWalletModelDataList();
                                    ...
                                    while(iterator.hasNext()) {
                                        ...
                                        if (perWalletModelData != null && perWalletModelData.getWalletFilename() != null) {
                                            if (perWalletModelData.getWalletFilename().equals(selectedWalletFilename)) {
                                                ...
                                                this.fastcoinController.getModel().setActiveWalletByFilename(selectedWalletFilename);
                                                ...
                                                if ((perWalletModelData.getWalletFilename() + "." + BackupManager.FILE_ENCRYPTED_WALLET_SUFFIX).equals(selectedWalletFilename)) {
                                                    walletIsAlreadyOpen = true;
                                                    this.fastcoinController.getModel().setActiveWalletByFilename(perWalletModelData.getWalletFilename());
                                                    controller.fireDataChangedUpdateNow();
                                                    break;
                                                }    
                                            }
                        ...
                        if (!walletIsAlreadyOpen) {
                            ...
                            Message openMessage = new Message(controller.getLocaliser().getString("fastcoinWallet.openingWallet", new Object[]{selectedWalletFilename}));
    ...
    private CharSequence getPasswordFromUser() {
        ...
        FastcoinWalletLabel explainLabel = new FastcoinWalletLabel("");
        ...
        FastcoinWalletLabel passwordLabel = new FastcoinWalletLabel("");
        ...
        SwingWorker<Boolean, Void> worker = new SwingWorker<Boolean, Void>() {
            ...
            protected Boolean doInBackground() throws Exception {
                try {
                    ...
                    WalletData perWalletModelData = fastcoinController.addWalletFromFilename(selectedWalletFilenameFinal);
 
                    log.debug("Setting active wallet for file '" + selectedWalletFilenameFinal + "'.");
                    fastcoinController.getModel().setActiveWalletByFilename(selectedWalletFilenameFinal);
                    ...
                    message = controller.getLocaliser().getString("fastcoinWallet.openingWalletIsDone", new Object[]{selectedWalletFilenameFinal});
            ...
            protected void done() {
                try {
                    ...
                    if (wasSuccessful) {
                        ...
                        WalletData perWalletModelData = fastcoinController.getModel().getActivePerWalletModelData();
                        ...
                        if (lastBlockSeenHeight > 0 && currentChainHeight > 0) {
                            if (lastBlockSeenHeight >= currentChainHeight) {
                                // FastcoinWallet is at or ahead of current chain - but check there isnt a replay at the moment.
                                ...
                                // FastcoinWallet is behind the current chain - need to sync.
                                needToSync = true;
                            }
                        } else {
                            ...
                            if (mainFrame != null) {
                                ...
                                if (walletListPanel != null) {
                                    ...
                                    if (singleWalletPanel != null) {
                                        singleWalletPanel.setSyncMessage(controller.getLocaliser().getString("fastcoinWalletDownloadListener.downloadingTextShort"), Message.NOT_RELEVANT_PERCENTAGE_COMPLETE);
                                    }
                                }
                            }
                        ...
                        WalletData loopData = fastcoinController.getModel().getPerWalletModelDataByWalletFilename(selectedWalletFilenameFinal);
                        if (loopData != null) {
