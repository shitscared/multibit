...
public class ShowPreferencesSubmitAction extends AbstractAction {
     ...
     */
    public ShowPreferencesSubmitAction(FastcoinController fastcoinController, ExchangeController exchangeController, PreferencesDataProvider dataProvider, Icon icon,
            FastcoinWalletFrame mainFrame) {
        super(fastcoinController.getLocaliser().getString("showPreferencesSubmitAction.text"), icon);
...
//                controller.getModel().setUserPreference(FastcoinModel.PREVIOUS_SEND_FEE, previousSendFee);
//
//                // Check fee is set.
//                if (newSendFee == null || "".equals(newSendFee)) {
//                    // Fee must be set validation error.
//                    controller.getModel().setUserPreference(FastcoinModel.SEND_FEE, previousSendFee);
...
//                        if (feeAsBigInteger.compareTo(FastcoinModel.SEND_MINIMUM_FEE) < 0) {
//                            feeValidationError = true;
//                            updateStatusText = controller.getLocaliser().getString(
//                                    "showPreferencesPanel.feeCannotBeSmallerThanMinimumFee");
//                        } else {
//                            if (feeAsBigInteger.compareTo(FastcoinModel.SEND_MAXIMUM_FEE) >= 0) {
//                                feeValidationError = true;
//                                updateStatusText = controller.getLocaliser().getString(
//                                        "showPreferencesPanel.feeCannotBeGreaterThanMaximumFee");
//                            } else {
//                                // Fee is ok.
//                                controller.getModel().setUserPreference(FastcoinModel.SEND_FEE, newSendFee);
//                            }
//                        }
...
//                }
            ...
            if (openUriDialog != null) {
                controller.getModel().setUserPreference(FastcoinModel.OPEN_URI_SHOW_DIALOG, openUriDialog);
            }

            // Open URI - use URI.
            String openUriUseUri = dataProvider.getOpenUriUseUri();
            if (openUriUseUri != null) {
                controller.getModel().setUserPreference(FastcoinModel.OPEN_URI_USE_URI, openUriUseUri);
            }
            ...
            boolean showFastcoinConvertedToFiat = dataProvider.getNewShowFastcoinConvertedToFiat();
            ...
            } else if (dataProvider.getPreviousShowFastcoinConvertedToFiat() != showFastcoinConvertedToFiat) {
                wantToFireDataStructureChanged = true;
                if (showFastcoinConvertedToFiat) {
                    restartTickerTimer = true;
                }
            ...
            controller.getModel().setUserPreference(ExchangeModel.SHOW_FASTCOIN_CONVERTED_TO_FIAT,
              Boolean.valueOf(showFastcoinConvertedToFiat).toString());
            ...
            if (newCurrency1 != null && !newCurrency1.equals(previousCurrency1)) {
                ...
                if (ExchangeData.FASTCOIN_CHARTS_EXCHANGE_NAME.equals(newExchange1)) {
            ...
            if (wantToFireDataStructureChanged || lookAndFeelHasChanged) {
                ...
                // Switch off blinks.
                this.fastcoinController.getModel().setBlinkEnabled(false);
                ...
                try {
                    ...
                    this.fastcoinController.getModel().setBlinkEnabled(true);
                }            
            }
