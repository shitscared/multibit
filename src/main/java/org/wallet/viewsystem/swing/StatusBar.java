...
import org.wallet.message.MessageListener;
import org.wallet.model.core.StatusEnum;
import org.wallet.viewsystem.swing.action.FastcoinWalletAction;
 ...
 * <p/>
 * In FastcoinWallet the StatusBar is responsible for :
 * <p/>
...
public class StatusBar extends JPanel implements MessageListener {
  ...
  final private FastcoinWalletButton statusLabel;
   ...
   */
  public StatusBar(FastcoinController fastcoinController, FastcoinWalletFrame mainFrame) {
    ...
    final FastcoinController finalController = this.fastcoinController;
    ...
    onlineLabel.addMouseListener(new MouseAdapter() {
      ...
      public void mouseEntered(MouseEvent arg0) {
        int blockHeight = -1;
        if (finalController.getFastcoinWalletService() != null) {
          if (finalController.getFastcoinWalletService().getChain() != null) {
            if (finalController.getFastcoinWalletService().getChain().getChainHead() != null) {
              blockHeight = finalController.getFastcoinWalletService().getChain().getChainHead().getHeight();
              Block blockHeader = finalController.getFastcoinWalletService().getChain().getChainHead().getHeader();
              Date blockTime = blockHeader.getTime();
              int numberOfPeers = 0;
              if (finalController.getFastcoinWalletService().getPeerGroup() != null && finalController.getFastcoinWalletService().getPeerGroup().getConnectedPeers() != null) {
                numberOfPeers = finalController.getFastcoinWalletService().getPeerGroup().getConnectedPeers().size();
              }
              onlineLabel.setToolTipText(HelpContentsPanel.createMultilineTooltipText(new String[]{
                      finalController.getLocaliser().getString("fastcoinWalletFrame.numberOfBlocks",
                              new Object[]{blockHeight}),
                      finalController.getLocaliser().getString("fastcoinWalletFrame.blockDate",
                              new Object[]{dateFormatter.format(blockTime)}),
                      finalController.getLocaliser().getString("fastcoinWalletFrame.connectedTo",
    ...
    statusLabel = new FastcoinWalletButton("");
    ...
    FastcoinWalletAction showMessagesAction = new FastcoinWalletAction(controller, null, null,
            "messagesPanel.title", "messagesPanel.mnemonic", org.wallet.viewsystem.View.MESSAGES_VIEW);
    ...
    String tooltipText = HelpContentsPanel.createMultilineTooltipText(new String[]{
            controller.getLocaliser().getString("fastcoinWalletFrame.statusBar.tooltip1"), "\n",
            controller.getLocaliser().getString("fastcoinWalletFrame.statusBar.tooltip2")});
    ...
    int onlineWidth = Math.max(Math.max(
            getFontMetrics(FontSizer.INSTANCE.getAdjustedDefaultFont()).stringWidth(
                    controller.getLocaliser().getString("fastcoinWalletFrame.onlineText")),
            getFontMetrics(FontSizer.INSTANCE.getAdjustedDefaultFont()).stringWidth(
                    controller.getLocaliser().getString("fastcoinWalletFrame.offlineText"))),
            getFontMetrics(FontSizer.INSTANCE.getAdjustedDefaultFont()).stringWidth(
                    controller.getLocaliser().getString("fastcoinWalletFrame.errorText"))
    )
  ...
  public void updateOnlineStatusTextOnSwingThread(final StatusEnum finalStatusEnum) {
    ...
    if (finalStatusEnum == StatusEnum.ONLINE) {
      onlineLabel.setForeground(new Color(0, 170, 0));
      if (mainFrame != null) {
        BlinkLabel estimatedBalanceFSTLabel = mainFrame.getEstimatedBalanceFSTLabel();
        if (estimatedBalanceFSTLabel != null) {
          estimatedBalanceFSTLabel.setBlinkEnabled(true);
        }
    ...
    if (finalStatusEnum == StatusEnum.ERROR) {
      ...
      String toolTip = HelpContentsPanel.createMultilineTooltipText(new String[]{
              controller.getLocaliser().getString("fastcoinWalletFrame.statusBar.error1"),
              controller.getLocaliser().getString("fastcoinWalletFrame.statusBar.error2")});
