...
public class PrivateKeyFileFilter extends javax.swing.filechooser.FileFilter {
    ...
    public boolean accept(File file) {
        return (file.isDirectory() && !(file.getName().toLowerCase().endsWith(MAC_APPLICATION_SUFFIX))) || (file.getName().toLowerCase().endsWith(FastcoinModel.PRIVATE_KEY_FILE_EXTENSION));
    }

    @Override
    public String getDescription() {
        String fastcoinWalletText = controller.getLocaliser().getString("fastcoinWalletFrame.title");
        return fastcoinWalletText + " (*." + FastcoinModel.PRIVATE_KEY_FILE_EXTENSION + ")";
    }
}