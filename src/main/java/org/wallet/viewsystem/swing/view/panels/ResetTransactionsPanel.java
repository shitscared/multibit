...
public class ResetTransactionsPanel extends JPanel implements Viewable, WalletBusyListener {
     ...
     */
    public ResetTransactionsPanel(FastcoinController fastcoinController, FastcoinWalletFrame mainFrame) {
    ...
    private JPanel createExplainPanel(int stentWidth) {
        FastcoinWalletTitledPanel explainPanel = new FastcoinWalletTitledPanel(controller.getLocaliser().getString(
        ...
        FastcoinWalletTitledPanel.addLeftJustifiedTextAtIndent(" ", 3, explainPanel);

        String explainText1 = controller.getLocaliser().getString("resetTransactionsPanel.explainLabel.text1");
        FastcoinWalletTextArea explainTextArea1 = new FastcoinWalletTextArea(explainText1, 2, 40, controller);
        ...
        constraints.gridwidth = 1;
        constraints.anchor = GridBagConstraints.LINE_START;
        explainPanel.add(FastcoinWalletTitledPanel.createStent(stentWidth), constraints);
        ...
        constraints.gridwidth = 1;
        constraints.anchor = GridBagConstraints.CENTER;
        explainPanel.add(FastcoinWalletTitledPanel.createStent(FastcoinWalletTitledPanel.SEPARATION_BETWEEN_NAME_VALUE_PAIRS), constraints);
    ...
    private JPanel createButtonPanel() {
        ...
        resetTransactionsSubmitAction = new ResetTransactionsSubmitAction(this.fastcoinController, mainFrame,
                ImageLoader.createImageIcon(ImageLoader.RESET_TRANSACTIONS_ICON_FILE));
        FastcoinWalletButton submitButton = new FastcoinWalletButton(resetTransactionsSubmitAction, controller);
    ...
    public void walletBusyChange(boolean newWalletIsBusy) {       
        ...
        if (this.fastcoinController.getModel().getActivePerWalletModelData().isBusy()) {
            ...
            resetTransactionsSubmitAction.putValue(Action.SHORT_DESCRIPTION, HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("fastcoinWalletSubmitAction.walletIsBusy",
