...
import org.wallet.network.FastcoinWalletDownloadListener;
...
import org.wallet.viewsystem.swing.view.components.BlinkLabel;
import org.wallet.viewsystem.swing.view.components.FontSizer;
import org.wallet.viewsystem.swing.view.components.FastcoinWalletTextField;
...
public class SingleWalletPanel extends JPanel implements ActionListener, FocusListener, WalletBusyListener {
  ...
  private FastcoinWalletTextField walletDescriptionTextField;
  private Border walletDescriptionTextFieldBorder;

  private BlinkLabel amountLabelFST;
  ...
  public SingleWalletPanel(WalletData perWalletModelData, final FastcoinController fastcoinController, FastcoinWalletFrame mainFrame, final WalletListPanel walletListPanel) {
    ...
    perWalletModelData.setSingleWalletDownloadListener(new SingleWalletPanelDownloadListener(this.fastcoinController, this));
    ...
    walletDescriptionTextField = new FastcoinWalletTextField(perWalletModelData.getWalletDescription(), WIDTH_OF_TEXT_FIELD,
            controller);
    ...
    myRoundedPanel.add(FastcoinWalletTitledPanel.createStent(2, 2), constraints);
    ...
    constraints.gridheight = 1;
    constraints.anchor = GridBagConstraints.LINE_END;
    JPanel iconRowStent = FastcoinWalletTitledPanel.createStent((int) (walletDescriptionTextFieldBorder.getBorderInsets(this).left * 0.5));
    myRoundedPanel.add(iconRowStent, constraints);

    // FastcoinWallet type icon.
    ...
    twistyStent = FastcoinWalletTitledPanel.createStent(1, 1);
    ...
    amountLabelFST = new BlinkLabel(controller, false);
    amountLabelFST.setBackground(ColorAndFontConstants.BACKGROUND_COLOR);
    amountLabelFST.setBorder(BorderFactory.createEmptyBorder(0, 3, 0, 3));
    amountLabelFST.setBlinkEnabled(true);
    amountLabelFST.setHorizontalAlignment(JLabel.TRAILING);
    ...
    constraints.gridheight = 1;
    constraints.anchor = GridBagConstraints.LINE_END;
    myRoundedPanel.add(amountLabelFST, constraints);
  ...
  public static int calculateNormalWidth(JComponent component) {
    ...
    return (int) (fontMetrics.stringWidth(FastcoinWalletFrame.EXAMPLE_MEDIUM_FIELD_TEXT) + WIDTH_DELTA);
  }
  ...
  public void addMouseListener(MouseListener mouseListener) {
    ...
    amountLabelFST.addMouseListener(mouseListener);
  ...
  public void setBusyIconStatus(boolean isBusy) {
    ...
    if (this.fastcoinController.getModel().getActivePerWalletModelData().isBusy()) {
      ...
      if (perWalletModelData != null) {
        String toolTipText = HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("fastcoinWalletSubmitAction.walletIsBusy",
  ...
  private void saveChanges() {
    if (!perWalletModelData.isFilesHaveBeenChangedByAnotherProcess()) {
      ...
      String titleText = controller.getLocaliser().getString("fastcoinWalletFrame.title");
      if (this.fastcoinController.getModel().getActiveWallet() != null) {
        titleText = titleText + FastcoinWalletFrame.SEPARATOR
                + this.fastcoinController.getModel().getActivePerWalletModelData().getWalletDescription() + FastcoinWalletFrame.SEPARATOR
                + this.fastcoinController.getModel().getActivePerWalletModelData().getWalletFilename();
      }
  ...
  public void updateFromModel(boolean blinkEnabled, boolean useBusyStatus) {
    ...
    String balanceTextToShowFST = controller.getLocaliser().fastcoinValueToString(estimatedBalance, true, false);
    ...
    if (useBusyStatus && perWalletModelData.isBusy()) {
      ...
      if (amountLabelFST != null && amountLabelFST.getText() != null && !"".equals(amountLabelFST.getText())
              && !balanceTextToShowFST.equals(amountLabelFST.getText()) && blinkEnabled) {
        amountLabelFST.blink(balanceTextToShowFST);
        amountLabelFiat.blink(balanceTextToShowFiat);
      } else {
        amountLabelFST.setText(balanceTextToShowFST);
    ...
    if (perWalletModelData.isFilesHaveBeenChangedByAnotherProcess()) {
      ...
      walletDescriptionTextField.setEditable(false);
      amountLabelFST.setText("");
      amountLabelFST.setEnabled(false);
  ...
  public void setSyncMessage(String message, double syncPercent) {
    ...
    if (syncPercent > FastcoinWalletDownloadListener.DONE_FOR_DOUBLES) {
      ...
      amountLabelFST.setBlinkEnabled(false);
      amountLabelFiat.setBlinkEnabled(false);

      amountLabelFST.setText(message);
      amountLabelFiat.setText(percentText);

      amountLabelFST.setBlinkEnabled(true);
      amountLabelFiat.setBlinkEnabled(true);

      if (perWalletModelData.getWalletFilename().equals(this.fastcoinController.getModel().getActiveWalletFilename())) {
