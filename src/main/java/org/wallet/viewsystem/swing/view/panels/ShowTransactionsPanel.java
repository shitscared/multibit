...
import org.wallet.viewsystem.swing.UpdateTransactionsTimerTask;
import org.wallet.viewsystem.swing.WalletTableModel;
...
import org.wallet.viewsystem.swing.action.ShowTransactionDetailsAction;
...
public class ShowTransactionsPanel extends JPanel implements Viewable, CurrencyConverterListener {
    ...
    private FastcoinWalletButton showTransactionsButton;
    
    private Action exportTransactionsSubmitAction;
    private FastcoinWalletButton exportTransactionsButton;
    ...
    public ShowTransactionsPanel(FastcoinController fastcoinController, FastcoinWalletFrame mainFrame) {
    ...
    private JPanel createTransactionsPanel() {
        ...
        walletTableModel = new WalletTableModel(fastcoinController);
        ...
        tableColumn = table.getColumnModel().getColumn(3); // Amount (FST).
        int amountFSTWidth = Math.max(fontMetrics.stringWidth(controller.getLocaliser().getString("sendFastcoinPanel.amountLabel") + " (FST)"),
                fontMetrics.stringWidth("00000.000000000"));
        tableColumn.setPreferredWidth(amountFSTWidth);
        tableColumn.setMinWidth(amountFSTWidth);

        if (CurrencyConverter.INSTANCE.isShowingFiat()) {
            tableColumn = table.getColumnModel().getColumn(4); // Amount (fiat).
            int amountFiatWidth = Math.max(fontMetrics.stringWidth(controller.getLocaliser().getString("sendFastcoinPanel.amountLabel") + " (USD)"),
    ...
    private JPanel createButtonPanel() {
        ...
        showTransactionDetailsAction = new ShowTransactionDetailsAction(fastcoinController, mainFrame, this);
        showTransactionsButton = new FastcoinWalletButton(showTransactionDetailsAction, controller);
        ...
        exportTransactionsSubmitAction = new ExportTransactionsSubmitAction(fastcoinController, mainFrame);
        exportTransactionsButton = new FastcoinWalletButton(exportTransactionsSubmitAction, controller);
        showTransactionsButton.setEnabled(false);
        constraints.fill = GridBagConstraints.NONE;
        constraints.gridx = 2;
        constraints.gridy = 0;
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row,
            ...
            case BUILDING: {
                if (fastcoinController.getFastcoinWalletService().getChain() == null) {
                    ...
                    int numberOfBlocksEmbedded = fastcoinController.getFastcoinWalletService().getChain().getBestChainHeight() - confidence.getAppearedAtChainHeight() + 1;
                    if (transaction != null && transaction.isCoinBase()) {
                    ...
                    if (numberOfBlocksEmbedded >= 6) {
                        primaryLabel.setToolTipText(HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("fastcoinWalletFrame.status.isConfirmed")));
                    } else {
                        if (transaction != null && transaction.isCoinBase()) {
                            primaryLabel.setToolTipText(HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("fastcoinWalletFrame.status.beingConfirmedAndCoinbase")));
                        } else {
                            primaryLabel.setToolTipText(HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("fastcoinWalletFrame.status.beingConfirmed")));
                        }
                    }
                }
                break;
            ...
            case DEAD: {
                ...
                primaryLabel.setText(controller.getLocaliser().getString("fastcoinWalletFrame.status.dead"));
                break;
            }
            default: {
                primaryLabel.setIcon(null);
        ...
        private String getUnconfirmedConfidenceToolTip(Transaction transaction) {
            ...
            if (transaction != null) {
                if (transaction.getLockTime() > 0) {
                    ...
                    transactionTrustfulness = FastcoinWallet.getController().getLocaliser().getString("fastcoinWalletFrame.status.notConfirmedAndNotStandard") + ".";
                } else {
                    // Normal transaction.
                    if (transaction.isCoinBase()) {
                        transactionTrustfulness = FastcoinWallet.getController().getLocaliser().getString("fastcoinWalletFrame.status.notConfirmedAndCoinbase") + ".";
                    } else {
                        transactionTrustfulness = FastcoinWallet.getController().getLocaliser().getString("fastcoinWalletFrame.status.notConfirmed") + ".";
                    }
                }
            }
    ...
    class TrailingJustifiedNumericRenderer extends DefaultTableCellRenderer {
        ...
        FastcoinWalletLabel label;

        public TrailingJustifiedNumericRenderer() {
            label = new FastcoinWalletLabel("");
        }
    ...
    class DecimalAlignRenderer implements TableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row,
            ...
            if (value == null) {
                pane.setText("\t" + controller.getLocaliser().fastcoinValueToString(BigInteger.ZERO, false, false));
            } else {
