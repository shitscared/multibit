...
import org.wallet.viewsystem.swing.action.SendFastcoinNowAction;
...
public class SendFastcoinConfirmPanel extends JPanel implements WalletBusyListener {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                if (thisPanel != null && thisPanel.isVisible()) {
                    final FastcoinController fastcoinController = FastcoinWallet.getFastcoinController();
                    if (fastcoinController != null) {
                        ...
                        if (enableSend) {
                            if (confirmText1 != null) {
                                if (FastcoinWallet.getController().getLocaliser()
                            ...
                            if (confirmText1 != null) {
                                confirmText1.setText(FastcoinWallet.getController().getLocaliser()
    ...
    private String getConfidenceToolTip(int numberOfPeers) {
        ...
        if (numberOfPeers == 0) {
            builder.append(FastcoinWallet.getController().getLocaliser().getString("transactionConfidence.seenByUnknownNumberOfPeers"));
        } else {
            builder
