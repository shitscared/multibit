...
import com.google.fastcoin.core.DownloadListener;
...
import org.wallet.viewsystem.swing.view.walletlist.SingleWalletPanelDownloadListener;
 ...
 * 
 */
public class FastcoinWalletDownloadListener extends DownloadListener {
    private static final Logger log = LoggerFactory.getLogger(FastcoinWalletDownloadListener.class);
    ...
    public FastcoinWalletDownloadListener(FastcoinController controller) {
        this.fastcoinController = controller;
    ...
    protected void progress(double pct, int blocksSoFar, Date date) {
        if (pct > DONE_FOR_DOUBLES) {
            ...
            synchronized (lockObject) {
                String downloadStatusText = controller.getLocaliser().getString("fastcoinWalletDownloadListener.startDownloadTextShort",
                        new Object[] {blocksSoFar})
                        + " "
                        + controller.getLocaliser().getString(
                                "fastcoinWalletDownloadListener.blockDateText",
                ...
                if (!(downloadStatusText.indexOf("fastcoinWalletDownloadListener") > -1)) {
        ...
        if (blocks == 0) {
            ...
            synchronized (lockObject) {
                ...
                if (blocks <= CRITERIA_LARGE_NUMBER_OF_BLOCKS) {
                    startDownloadText = controller.getLocaliser().getString("fastcoinWalletDownloadListener.startDownloadTextShort",
                            new Object[] {blocks});
                } else {
                    startDownloadText = controller.getLocaliser().getString("fastcoinWalletDownloadListener.startDownloadTextLong",
                            new Object[] {blocks});
                }

                String startDownloadTextForLabel = controller.getLocaliser().getString("fastcoinWalletDownloadListener.downloadingText");

                // When busy occasionally the localiser fails to localise
                if (!(startDownloadText.indexOf("fastcoinWalletDownloadListener") > -1)) {
    ...
    protected void doneDownload() {
        ...
        String downloadStatusText = controller.getLocaliser().getString("fastcoinWalletDownloadListener.doneDownloadText");
