...
/**
 * FastcoinWallet info is the companion info to the fastcoinj FastcoinWallet that wallet uses
...
public class WalletInfoData {
    ...
    private static final String INFO_MAGIC_TEXT = "fastcoinWallet.info";
    ...
    private FastcoinWalletVersion walletVersion;
    private com.google.fastcoin.core.Wallet wallet;
     ...
     */
    public WalletInfoData(String walletFilename, com.google.fastcoin.core.Wallet wallet, FastcoinWalletVersion walletVersion) {
        ...
        if (wallet != null) {
            for (ECKey key : wallet.getKeys()) {
                if (receivingAddress.getAddress().equals(
                        key.toAddress(FastcoinWallet.getFastcoinController().getModel().getNetworkParameters()).toString())) {
     ...
     */
    public void checkAllReceivingAddressesAppearInWallet(com.google.fastcoin.core.Wallet wallet) {
     ...
     */
    public void writeToFile(String walletInfoFilename, FastcoinWalletVersion walletVersion) throws WalletSaveException {
        ...
        try {
            ...
            // Write out the wallet addressbook identifier.
            ...
            walletPreferencesClone.remove(FastcoinModel.WALLET_FILE_SIZE);
            walletPreferencesClone.remove(FastcoinModel.WALLET_FILE_LAST_MODIFIED);
            walletPreferencesClone.remove(FastcoinModel.WALLET_INFO_FILE_SIZE);
            walletPreferencesClone.remove(FastcoinModel.WALLET_INFO_FILE_LAST_MODIFIED);

            walletPreferencesClone.remove(FastcoinModel.VALIDATION_ADDRESS_IS_INVALID);
            walletPreferencesClone.remove(FastcoinModel.VALIDATION_ADDRESS_VALUE);
            walletPreferencesClone.remove(FastcoinModel.VALIDATION_AMOUNT_IS_INVALID);
            walletPreferencesClone.remove(FastcoinModel.VALIDATION_AMOUNT_IS_MISSING);
            walletPreferencesClone.remove(FastcoinModel.VALIDATION_AMOUNT_IS_NEGATIVE_OR_ZERO);
            walletPreferencesClone.remove(FastcoinModel.VALIDATION_AMOUNT_VALUE);
            walletPreferencesClone.remove(FastcoinModel.VALIDATION_NOT_ENOUGH_FUNDS);

            walletPreferencesClone.remove(FastcoinModel.SEND_PERFORM_PASTE_NOW);
    ...
    public void loadFromFile() {
        ...
        try {
            ...
            if (firstLine == null) {
                // This is not an wallet address book.
            ...
            if (walletVersionTokenNumber == 2) {
                ...
                if (!WALLET_VERSION_MARKER.equals(walletVersionMarker)
                        || !(FastcoinWalletVersion.SERIALIZED.getWalletVersionString().equals(walletVersionString)
                                || FastcoinWalletVersion.PROTOBUF.getWalletVersionString().equals(walletVersionString) || FastcoinWalletVersion.PROTOBUF_ENCRYPTED
                    ...
                    if (!walletVersion.getWalletVersionString().equals(walletVersionString)) {
                        ...
                        if (FastcoinWalletVersion.SERIALIZED.getWalletVersionString().equals(walletVersionString)) {
                            walletVersion = FastcoinWalletVersion.SERIALIZED;
                        } else if (FastcoinWalletVersion.PROTOBUF.getWalletVersionString().equals(walletVersionString)) {
                            walletVersion = FastcoinWalletVersion.PROTOBUF;
                        } else if (FastcoinWalletVersion.PROTOBUF_ENCRYPTED.getWalletVersionString().equals(walletVersionString)) {
                            walletVersion = FastcoinWalletVersion.PROTOBUF_ENCRYPTED;
                        }
                    }
                }
            } else {
    ...
    public static String createWalletInfoFilename(String walletFilename) {
        ...
        if (walletFilename.endsWith(FastcoinModel.WALLET_FILE_EXTENSION)) {
            walletInfoFilename = walletInfoFilename.substring(0,
                    walletFilename.length() - FastcoinModel.WALLET_FILE_EXTENSION.length() - 1);
    ...
    public static String decodeURLString(String stringToDecode) {
        try {
            // Earlier fastcoin-wallets did not encode the info file - check if
            // encoded.
    ...
    public FastcoinWalletVersion getWalletVersion() {
        return walletVersion;
    }

    public void setWalletVersion(FastcoinWalletVersion walletVersion) {
        this.walletVersion = walletVersion;
    }
}
