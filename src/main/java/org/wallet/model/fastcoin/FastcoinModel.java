...
import org.wallet.model.AbstractModel;
import org.wallet.model.ModelEnum;
...
/**
 * Model containing the FastcoinWallet data.
 * 
 ...
 * 
 */
public class FastcoinModel extends AbstractModel<CoreModel> {

      private static final Logger log = LoggerFactory.getLogger(FastcoinModel.class);

    // Constants used in the wallet.properties.

    // FastcoinWallet start up.
    ...
    // The number of serialised and protobuf2 wallets in the wallet.properties.
    ...
    // Send fastcoin and send fastcoin confirm.
    ...
    public static final BigInteger SEND_FEE_DEFAULT = new BigInteger("5000000");
    public static final BigInteger SEND_FEE_PER_KB_DEFAULT = new BigInteger("1000000");
    
    // Minimum fee.
    public static final BigInteger SEND_MINIMUM_FEE = new BigInteger("10000");

    // Maximum fee.
    public static final BigInteger SEND_MAXIMUM_FEE = new BigInteger("100000000"); // 1 FST.

    // Receive fastcoin.
    public static final String IS_RECEIVE_FASTCOIN = "isReceiveFastcoin";
    ...
    public FastcoinModel(CoreModel coreModel) {
        super(coreModel);
    ...
    public void setActiveWalletPreference(String key, String value) {
        if (FastcoinModel.SEND_AMOUNT.equals(key)) {
            if (value.contains(",")) {
                boolean bad = true;
                bad = !bad;
            }
     ...
     */
    public List<WalletTableData> getActiveWalletData() {
     ...
     */
    public WalletInfoData getActiveWalletInfo() {
     ...
     */
    public WalletData addWallet(final FastcoinController fastcoinController, Wallet wallet, String walletFilename) {
        if (walletFilename == null) {
            return null;
        }
        ...
        if (wallet != null) {
            wallet.addEventListener(fastcoinController);
        }

        createWalletTableData(fastcoinController, walletFilename);
     ...
     */
    public ArrayList<WalletTableData> createActiveWalletData(final FastcoinController fastcoinController) {
        return createWalletTableData(fastcoinController, this.getActivePerWalletModelData());
    }
     ...
     * to show to the user in tabular form.
     */
    public ArrayList<WalletTableData> createWalletTableData(final FastcoinController fastcoinController, String walletFilename) {
        ...
        return createWalletTableData(fastcoinController, perWalletModelData);
    }

    public ArrayList<WalletTableData> createWalletTableData(final FastcoinController fastcoinController, WalletData perWalletModelData) {
        ArrayList<WalletTableData> walletData = new ArrayList<WalletTableData>();
        ...
        if (transactions != null) {
            for (Transaction loopTransaction : transactions) {
                ...
                if (transactionInputs != null) {
                    ...
                    if (firstInput != null) {
                        walletDataRow.setDescription(createDescription(fastcoinController, perWalletModelData.getWallet(), transactionInputs,
                                transactionOutputs, walletDataRow.getCredit(), walletDataRow.getDebit()));
                    }
                }
                walletDataRow.setDate(createDate(fastcoinController, loopTransaction));
     ...
     */
    private Date createDate(final FastcoinController fastcoinController, Transaction transaction) {
                ...
                if (iterator.hasNext()) {
                    ...
                    try {
                        if (fastcoinController != null && fastcoinController.getFastcoinWalletService() != null
                                && fastcoinController.getFastcoinWalletService().getBlockStore() != null) {
                            appearsInStoredBlock = fastcoinController.getFastcoinWalletService().getBlockStore().get(appearsInHash);
    ...
    public NetworkParameters getNetworkParameters() {
        ...
        String testOrProduction = super.getUserPreference(FastcoinModel.TEST_OR_PRODUCTION_NETWORK);
        if (testOrProduction == null) {
            testOrProduction = FastcoinModel.PRODUCTION_NETWORK_VALUE;
            super.setUserPreference(FastcoinModel.TEST_OR_PRODUCTION_NETWORK, testOrProduction);
        }
        if(FastcoinModel.TESTNET3_VALUE.equalsIgnoreCase(testOrProduction)) {
