...
public class QRCodeGenerator {
    ...
    public QRCodeGenerator(FastcoinController fastcoinController) {
     ...
     * 
     * @param address
     *            Fastcoin address to show
     * @param amount
     *            amount of FST to show - text
     * @param label
     *            label for swatch
     * @return
     */
    public BufferedImage generateQRcode(String address, String amount, String label, int scaleFactor) {
        ...
        try {
            ...
            if (address != null && !"".equals(address) && this.fastcoinController.getFastcoinWalletService() != null
                    && this.fastcoinController.getModel().getNetworkParameters() != null) {
                decodeAddress = new Address(this.fastcoinController.getModel().getNetworkParameters(), address);
            }
            if (decodeAddress != null && !"".equals(decodeAddress)) {
                if (amount != null && !"".equals(amount)) {
                    ...
                    if (converterResult.isFstMoneyValid()) {
                        fastcoinURI = FastcoinURI.convertToFastcoinURI(decodeAddress, converterResult.getFstMoney().getAmount().toBigInteger(), label, null);
                    } else {
                       // No parsable amount - show nothing.  
                    }                } else {
                    fastcoinURI = FastcoinURI.convertToFastcoinURI(decodeAddress, null, label, null);
                }
            }
            this.fastcoinController.getModel().setActiveWalletPreference(FastcoinModel.SEND_PERFORM_PASTE_NOW, "false");
        } catch (IllegalArgumentException e) {
            //log.warn("The address '" + address + "' could not be converted to a fastcoin address. (IAE)");
            return null;
        } catch (AddressFormatException e) {
            //log.warn("The address '" + address + "' could not be converted to a fastcoin address. (AFE)");
            return null;
        }
